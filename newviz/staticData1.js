...
var pelarstoken = getCookie("token");
...
function parsePhotos(multiData){
	...
	console.log(multiData.length+"multiData length - photos");
		...
		for(i=0; i<docuNote.length; i++){
			console.log("docutext " + docuNote[i]['data'])
			$.get(docuNote[i]['data'],function(data){docuTexts.push(data)})
		}
////online version
	// function processURL(){
	// 	for(i=0; i<docuNote.length; i++){
...
//         		.attr("opacity",1);
            ...
            docCom = svgMain
                ...
                .attr("x", timeXTrue(thisTime)-iconW/2 - (docuTexts[lIndex].length)*3.3 - 50)
				.attr("y", butY-10)
		      .attr("font-size",11)
		      .attr("dy", "1.7em")
				.attr("fill",darkColor)
                .text(function() {
                	console.log(docuTexts[lIndex]);
                	return docuTexts[lIndex];
                });
		        // .call(wrap2, iconW)
        		//.attr("opacity",1);
